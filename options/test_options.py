...
class CycleTestOptions(Options):
    ...
    def initialize(self, parser : argparse.ArgumentParser):
        ...
        parser.add_argument('--num_tests', default=float('inf'), help="max number of images to run on")
        parser.add_argument('--result_dir', default='./results', help='result directory')
        parser.add_argument('--save_separate', action='store_true', help="save all separate graphics")
        self.to_train = False
        return parser
