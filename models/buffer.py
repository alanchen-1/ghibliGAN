...
class Buffer():
    ...
    def query(self, fake_tensors : "list[torch.Tensor]"):
        ...
        Queries the buffer.
            Parameters:
                fake_tensors (list[torch.Tensor]) : list of fake tensors used for the query
            Returns:
                (torch.Tensor) : tensor of the queried tensors. non-deterministic due to random selection of images to add.
        """
        ...
        for fake_tensor in fake_tensors:
            ...
            p = random.random()
            if p < 0.5:
                # add random real tensor and replace it in the buffer so that D still only sees each image once
