import sys
sys.path.append('..')
from utils.str_to_layer import create_padding, create_norm
import torch.nn as nn


class ResNetBlock(nn.Module):
    """
    Class defining the building block of a residual
    (skip connection) convolutional network.
    """
    def __init__(
        self,
        dimension: int,
        ...
        Constructor for ResNetBlock.
            Parameters:
                ...
                dropout (bool) : whether to use Dropout layers or not
                bias (bool) : whether to use bias in the convolutional
                    layers or not
                norm (str) : normalization layer, default = 'instance'
                padding (str) : padding to use, helps reduce artifacts,
                    default = 'reflect'
...
class ResNetGenerator(nn.Module):
    """
    Class defining a Residual-based Generator with
    ...
    def __init__(
        ...
        Constructor for Residual Generator.
            Parameters:
                ...
                num_filters (int) : number of filters in initial and
                    last Convolutional layer, default = 64
                num_blocks (int) : number of ResNetBlocks to use, default = 6
                num_sampling (int) : number of intermediate downsampling
                    and upsampling layers, default = 2
                norm (str) : normalization layer, default = 'instance'
...
class PatchDiscriminator(nn.Module):
    ...
    PatchGAN is a discriminator architecture that outputs a value
    ...
    def __init__(
        ...
        Constructor for PatchGAN discriminator.
            Parameters:
                num_channels (int) : number of channels in input
                    (for RGB, num_channels = 3)
                num_filters (int) : number of filters in first
                    convolutional layer, default is 64
                num_conv_layers (int) : number of convolutional layers,
                    default is 3
                norm_layer (nn.Module) : normalization layer,
                    default is nn.InstanceNorm2d
                ker_size (int) : kernel size to use for convolutional layers,
                    default is 4
        ...
        for layer in range(1, num_conv_layers + 1):
            # set upper limit on size of layers
            out_filters = min(in_filters * 2, num_filters * 8)
