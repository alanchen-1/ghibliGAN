...
from models.init_nets import init_linear_lr, init_resnet_generator
from models.init_nets import init_patch_discriminator
...
class CycleGAN(nn.Module):
    def __init__(self, opt: Namespace, config: dict):
        """
            Parameters:
                ...
                config (dict) - config dictionary specifying the
                    network architectures, probably parsed from a yaml
        """
        ...
        self.genG = init_resnet_generator(
            self.in_channels,
            self.out_channels,
            use_gpu=opt.use_gpu,
            **config['model']['generator']
        )
        self.genF = init_resnet_generator(
            self.out_channels,
            self.in_channels,
            use_gpu=opt.use_gpu,
            **config['model']['generator']
        ...
        if self.to_train:
            # only define this other stuff if we are planning on
            # training the model
            # discriminator for X (D_X)
            self.netD_X = init_patch_discriminator(
                self.in_channels,
                **config['model']['discriminator'],
                use_gpu=opt.use_gpu
            )
            # discriminator for Y (D_Y)
            self.netD_Y = init_patch_discriminator(
                self.out_channels,
                **config['model']['discriminator'],
                use_gpu=opt.use_gpu
            )
    ...
    def forward(self):
        """
        Runs generator on real datasets and on the newly
        generated fake datasets.
        """
    ...
    def backward_D(
        ...
        Calculate the backward pass for an arbitrary discriminator.
            Parameters:
                ...
                factor (float) : factor to scale loss by
                    can be tweaked, default 0.5 as that is the original
                    CycleGAN paper implementation
            Returns:
    ...
    def backward_G(self):
        ...
        Currently uses discriminator loss, cycle loss, and identity loss
        if lambda_scaling is present.
        """
        # use options
        ...
        self.cycle_loss_Y = self.loss_cycle(
            self.fake_fake_Y, self.real_Y
        ) * lambda_Y  # G(F) lives in Y
        # cycle loss of ||F(G(X)) - X||
        self.cycle_loss_X = self.loss_cycle(
            self.fake_fake_X, self.real_X
        ) * lambda_X  # F(G) lives in X

        self.loss_G = self.loss_domain_X + self.loss_domain_Y + \
            self.cycle_loss_X + self.cycle_loss_Y + \
            self.identity_loss_X + self.identity_loss_Y
        self.loss_G.backward()

    def optimize(self):
        """
        Main method called every training loop.
        Forwards inputs and backpropagates losses.
        """
        # forward thru network
        self.forward()
    ...
    def general_setup(self):
        ...
        it is being run for training or not.
    ...
    def setup_schedulers(self):
        ...
        """
        self.schedulers = [
            init_linear_lr(optimizer, **self.config['train'])
    ...
    def eval(self):
        """
        Sets models in evaluation mode (turns off gradients, etc.)
